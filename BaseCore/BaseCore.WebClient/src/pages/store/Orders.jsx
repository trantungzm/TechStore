...
const getStatusBadgeClass = (status = '') => {
    ...
    if (status === 'Shipped' || status === 'Shipping') return 'bg-info text-dark';
...
const normalizeOrderDetailResponse = (payload) => {
    if (payload?.order) {
        return {
            order: {
                ...payload.order,
                timeline: payload.order.timeline || payload.order.timelines || [],
            },
    ...
    return {
        order: {
            ...payload,
            timeline: payload?.timeline || payload?.timelines || payload?.Timeline || [],
        },
...
const Orders = () => {
    ...
    const paymentLabel = (method) => {
        if (method === 'COD') return 'Thanh toÃ¡n khi nháº­n hÃ ng (COD)';
        if (method === 'StorePayment') return 'Thanh toÃ¡n táº¡i cá»­a hÃ ng';
        if (method === 'BankTransfer') return 'Chuyá»ƒn khoáº£n ngÃ¢n hÃ ng';
                                    ...
                                    <table className="table align-middle mb-0">
                                        ...
                                        <tbody>
                                            {pagedOrders.map((order) => (
                                                <tr key={order.id}>
                                                    <td className="fw-semibold">#{order.orderCode || order.id}</td>
