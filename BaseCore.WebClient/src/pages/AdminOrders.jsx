...
const statuses = ['Pending', 'Confirmed', 'Processing', 'ReadyForPickup', 'Shipping', 'Shipped', 'Completed', 'CancelRequested', 'Cancelled', 'CancelRejected'];
...
const AdminOrders = () => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        if (filterStatus) {
            // Đồng bộ bộ lọc hiển thị gom nhóm cho cả Shipping & Shipped
            if (filterStatus === 'Shipping' || filterStatus === 'Shipped') {
                items = items.filter((order) => order.status === 'Shipping' || order.status === 'Shipped');
            } else {
    ...
    return (
        <div className="px-4 py-6 lg:px-8">
            ...
            <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <button type="button" className="inline-flex items-center justify-center gap-2 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 text-sm font-semibold text-[var(--color-fg)] hover:bg-[var(--color-surface-2)] transition-colors" onClick={loadData}>
